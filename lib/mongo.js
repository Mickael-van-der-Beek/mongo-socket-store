  ...
  if(this.mongoClient._db && this.mongoClient._db.serverConfig.connected) {
    ...
    this.mongoClient.open(function (error, mongoClient) {
      if(error) {
        ...
        that.db = mongoClient.db('socket-store');
        that.db.createCollection('sessions', function (error, collection) {
          if(error) {
            throw error;
          }
          else {
...
MongoStore.prototype.publish = function () {};
...
MongoStore.prototype.subscribe = function () {};
...
MongoStore.prototype.unsubscribe = function () { };
...
Client.prototype.destroy = function (cb) {
  var that = this;
  this.store.collection.drop(function (error) {
    if(error) {
      cb.call(that, error);
    }
    else {
      that.store.db.createCollection(cb);
    }
  });
};
